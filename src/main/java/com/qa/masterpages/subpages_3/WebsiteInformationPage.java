package com.qa.masterpages.subpages_3;
...
public class WebsiteInformationPage extends TestBase {
	
	//Random String generated for : Document Number & all 
		String Common_No_random	= TestUtil.RandomStringGenerator();
		String Common_No_website = TestUtil.RandomWebsiteGenerator();
	...
	@FindBy(xpath= "//a[normalize-space()='Website Information']")
	WebElement WebsiteInformation_menu;
	...
	@FindBy(xpath= "//span[normalize-space()='Website information already exists']")
	...
	{
		data_inputfield.sendKeys(Common_No_website);
		description_inputfield.sendKeys(Common_No_random);
		save_button.click();
		
		try {
			Thread.sleep(3000);
			search_field.sendKeys(Common_No_website);
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		Assert.assertEquals(actual_text, Common_No_website);
